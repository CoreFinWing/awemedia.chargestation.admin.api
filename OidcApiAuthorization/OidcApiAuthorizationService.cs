...
namespace OidcApiAuthorization
{
    ...
    public class OidcApiAuthorizationService : IApiAuthorization
    {
        ...
        {
            _issuerUrl = Environment.GetEnvironmentVariable("b2c-IssuerUrl");
            _audience = Environment.GetEnvironmentVariable("b2c-Audience");
            ...
            HttpRequestHeaders httpRequestHeaders)
        {
            ...
            return await AuthorizeAsync(authorizationBearerToken);
        }
           ...
           string authorizationBearerToken)
        {
            ...
            while (apiAuthorizationResult == null)
            {
                ...
                try
                {
                    ...
                    openIdConnectConfiguration = await _oidcConfigurationManager.GetOpenIdConnectConfigurationAsync();
                    isserSigningKeys = openIdConnectConfiguration.SigningKeys;
                }
                catch (Exception ex)
                {
                    apiAuthorizationResult = new ApiAuthorizationResult(
                ...
                if (apiAuthorizationResult == null)
                {
                    ...
                    try
                    {
                        ...
                        var claimsPrinciple = _jwtSecurityTokenHandlerWrapper.ValidateToken(
                            authorizationBearerToken,
                            tokenValidationParameters);

                        // Successfully authorized.
                        apiAuthorizationResult = new ApiAuthorizationResult(claimsPrinciple);
                    }
        ...
        public async Task<ApiAuthorizationResult> AuthorizeAsync(
           HttpRequestHeaders httpRequestHeaders, string role, string roleClaimName = "extension_UserRoles")
        {
            var result = await this.AuthorizeAsync(httpRequestHeaders);
            if (result.Success)
            {
                var hasRequiredRole = result.ClaimsPrincipal?.Claims?.Any(s => s.Type.Equals(roleClaimName, StringComparison.OrdinalIgnoreCase) && s.Value.Equals(role, StringComparison.OrdinalIgnoreCase)) ?? false;
                result.FailureReason = hasRequiredRole ? null : "User doesn't have the required role";
            }
            return result;
        }

        public async Task<ApiAuthorizationResult> AuthorizeAsync(
          IHeaderDictionary httpRequestHeaders, string role, string roleClaimName = "extension_UserRoles")
        {
            var result = await this.AuthorizeAsync(httpRequestHeaders);
            if (result.Success)
            {
